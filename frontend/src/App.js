...
class App extends React.Component {
	constructor() {
		...
		this.fetchQuizEvaluation = this.fetchQuizEvaluation.bind(this);
		this.setIsFetchingAllQuestions = this.setIsFetchingAllQuestions.bind(this);
		this.setIsConfirm = this.setIsConfirm.bind(this);
	}
	...
	// 
	async fetchQuizEvaluation(quiz_id, user_responses) {
		const { url_header } = this.state;
		const url = `${url_header}/quizzes/${quiz_id}/evaluation/`;
		try {
			const res = await fetch(url, {
				method: 'POST',
				headers: {
                    'Content-Type': 'application/json',
					'Authorization': 'Token 7bf4b688105dee6d75c29abb4834a58c57c39b38'
                },
                body: JSON.stringify({
					"quiz_id": quiz_id,
			...
			const all_evaluations = await res.json()
			if (all_evaluations !== undefined) {
				this.setState({
					allEvaluation: all_evaluations
				})
			}
		} catch(error) {
			console.log(error)
			...
			<div className="App" style={overallAppStyle}>
				...
				{ (chosenQuizId === undefined) ?
					...
					<QuizBody 
						...
						setIsConfirm={this.setIsConfirm}
					/> : (!isFetchingAllQuestions && !isFetchingQuestion) && isConfirm !== undefined?
					<ResultsView
						...
						fetchQuizEvaluation={this.fetchQuizEvaluation}
